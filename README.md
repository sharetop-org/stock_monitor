...
- **全市场扫股回测**：拉取全部正常上市 A 股、过滤某年之前上市的，统一跑「N日新低买入」回测，统计「获利金额/总收益率/复合年化均>0」的股票占比，评估策略在老公司上的整体有效性。
- **「指定区间 + 固定买入价/卖出价」价格带回测**：在用户框定的 [开始, 结束] 区间内，前复权收盘价落入买入价/卖出价即触发买入/卖出，卖出后可反复再买；输出总收益、XIRR 复合年化、胜率、盈亏比、最大回撤与每笔买入/成交明细，可选导出 Excel。
...
│  ├─ strategy/        # 策略（N日新低回测 days_backtest.py、全市场扫股 screen_prefetch_backtest.py、价格带回测 price_band_backtest.py）
│  └─ backtest/        # 回测引擎
...
| `--low-days` | 新低回溯天数（交易日），默认 `1250`(≈5年)；跌破它即买入 |
| `--buy-amount` | 单次买入金额，默认 `10000`（买不起 1 手则硬买 1 手） |
| `--high-days` | 收盘价突破 N 日新高即卖出全部持仓（以统计胜率/盈亏比）；不传 = 只买不卖 |

### 3. 全市场股票扫描：200X年前上市公司的 N 日新低回测（`screen-backtest`）

用同一个「N 日新低买入」回测，**一次性扫描上千只历史股票的盈利表现**，统计「获利金额、总收益率、复合年化都大于 0」的股票数占总数比例。可用于评估该策略在"老牌上市公司"上的整体有效性。

**执行流程**：
1. 调 ShareTop `universes.get(list_status='L')` 拉取全部**正常上市**的 A 股（约 5600+ 只，不含退市/停牌）；
2. 用返回字段 `list_date`（`YYYYMMDD`) 过滤出**早于指定年份上市**的股票（默认 2008，可改 `--pre-year`）；
3. 对每只股票复用上一节 `days_backtest.backtest()`：凡跌破 N 日新低即分段买入、累积持仓，默认**只买不卖**（或可用 `--high-days` 变为"新低买入 + 突破新高卖出"），全程累积分红；
4. 逐股收集 **profit（获利金额） / ret（总收益率） / annual（复合年化）** 三项；
5. 统计并打印：**三项同时 >0 的股票数及其占比**，另附按上市年份分布、获利 TOP20、亏损样本数。

> ⚠️ **门槛高**：逐只回测要为每只股票各拉约 6 万根日K（前复权+不复权），1432 只 ≈ 数千次 ShareTop 请求，**耗时长且易触发限流**。务必先 `--limit` 小批试跑，正式全量建议后台/分批执行。

```bash
# 先小批验证（只扫 3 只）
python -m app.cli screen-backtest --limit 3 --low-days 250

# 全量：2008 年(默认)前上市的全部股票，1250 日新低买入，只买不卖
python -m app.cli screen-backtest --low-days 1250 --buy-amount 10000

# 只统计上证、最低 1000 只看 2000 年前上市的
python -m app.cli screen-backtest --pre-year 2000 --limit 100
...
| 参数 | 说明 |
|---|---|
| `--low-days` | 低位回看窗口（交易日），默认 `1250`（≈5年）；跌破它即触发买入 |
| `--buy-amount` | 单次买入金额，默认 `10000`（买不起一手则硬买一手） |
...
- **按上市年份分布**：每年上市样本数 / 平均胜率% / 平均复合年化%。
- **获利 TOP20**：按获利金额降序列出代码、名称、上市年、利润(万)、总收益%、复合年化%、胜率%。
- **亏损样本**：获利金额≤0 且 总收益率≤0 的股票只数，便于对照看整体策略在哪里失效。

**注意**：默认「只买不卖」时，`profit/ret/annual` 均为**期末按价结算的浮盈**口径（未实际卖出），分红已并入资产；若加 `--high-days`，盈亏才含真实卖出兑现的部分。

### 4. 指定区间 + 固定买入价/卖出价的价格带回测（`price-backtest`）

前面两个策略都依赖「N 日新低/新高」这类基于历史的相对信号。本节策略改为**纯价格阈值**：用户在 `[开始, 结束]` 区间内直接给出一个买入价和一个卖出价，当（前复权）收盘价 **≤ 买入价** 时买入一手仓位、**≥ 卖出价** 时卖出全部持仓；卖出后若再触买入价可**再次买入**（可反复），从而形成多笔独立交易用于统计胜率/盈亏比。

**策略口径**（与 `low-backtest` 一致的数据处理）：

| 环节 | 处理方式 |
|---|---|
| 回测区间 | `--start` 开始日（缺省=上市首日）；`--end` 结束日，**缺省=今日**（最后一根 K 线） |
| 买入信号 | 当日**前复权**收盘价 ≤ 买入价 |
| 卖出信号 | 当日**前复权**收盘价 ≥ 卖出价 |
| 成交价 | 按当日**不复权**真实收盘价成交；1 手=100 股取整，买不起一手则硬买一手（同 `low-backtest`） |
| 持仓模型 | **单仓位可反复**：卖出后再次触发买入价才重新买入，避免同价位反复堆积 |
| 分红/送转 | 已实施（状态 5）的现金分红计入现金、送转股并入持股数；只统计落在区间内的分红事件 |
| 指标 | 总收益、XIRR 复合年化、胜率、盈亏比、最大回撤、区间前复权最高/最低价、每笔买入明细、每笔成交盈亏明细 |

> ⚠️ **买/卖价是前复权口径**，且应满足 `买入价 < 卖出价`（否则同一天可能既触发买又触发卖，价差不成立，CLI 会报错）。由于前复权价随分红拆股逐年变化，建议结合当前价与历史区间实际估值来设定。

```bash
# 默认区间（上市首日 ~ 今日）：茅台前复权价 1200 以内买入、1800 以上卖出
python -m app.cli price-backtest --symbols 600519.SH --buy-price 1200 --sell-price 1800

# 指定区间 + 每笔加大到 10 万
python -m app.cli price-backtest --symbols 600519.SH,600036.SH \
    --start 2015-01-01 --buy-price 1200 --sell-price 1800 --buy-amount 100000

# 把逐股结果保存为 Excel（默认不保存，传入 --excel 才落盘）
python -m app.cli price-backtest --symbols 600519.SH --start 2015-01-01 \
    --buy-price 1200 --sell-price 1800 --excel data/price_backtest_result.xlsx
```
...
|---|---|
| `--symbols` | 股票代码，逗号分隔；缺省用自选股 |
| `--start` | 开始日期，如 `2015-01-01`；缺省=上市首日 |
| `--end` | 结束日期，如 `2024-12-31`；**缺省=今日（最后一根 K 线）** |
| `--buy-price` | 前复权买入触发价（元），默认 `10` |
| `--sell-price` | 前复权卖出触发价（元），默认 `20` |
| `--buy-amount` | 单次买入金额，默认 `10000`（买不起一手则硬买一手） |
| `--excel` | 导出逐股结果到该 Excel 文件，表头 `symbol / name / start / end / buy_price / sell_price / 买入次 / 投入本金 / 现金分红 / 总资产 / 利润(万) / 总收益% / 复合年化% / 胜率% / 盈亏比 / 最大回撤% / 区间前复权最高 / 区间前复权最低`（工作表名"回测结果"）；**缺省不保存** |

**输出物**：
- 汇总表：代码、名称、买入次、投入本金、现金分红、总资产、获利、总收益率(%)、复合年化(%)、胜率(%)、盈亏比、最大回撤(%)、区间前复权最高、区间前复权最低；
- 单只明细：区间、买入/卖出信号阈值、**区间前复权最高价/最低价**（含出现日期）、累计买入次数、投入本金、当前持股、现金分红、总资产、获利、总收益、复合年化(XIRR)、胜率/盈亏比/交易笔数、最大回撤，以及**每笔买入明细**（时间 + 前复权/不复权价、股数、花费）与**每笔成交盈亏明细**（买入日/卖出日/成本/了结价值/盈亏；`状态=已卖出` 为真实触发卖出，`状态=持有的` 表示期末未达卖出价、按当日收盘市值标注，非真实卖出）。
